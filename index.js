...
const checkForLatestTweet = async() => {
    ...
    const currentPersonOfInterest = peopleToFollow[counter][0]; //sets current person of interest to counter value
    const currentPersonOfInterestID = peopleToFollow[counter][1]; //sets current person of interest ID to counter value
    const currentPersonTweet = await getTweets(currentPersonOfInterest) //get current person's last tweet

    console.log('---------------------------------------------------------------') //to separate logs for more readability
    console.log(counter)
    console.log('Last tweet ID: ' + currentPersonTweet[0].id)
    console.log('Last tweet: ' + currentPersonTweet[0].text)
    console.log('')
    compareToFile(currentPersonTweet[0].id, currentPersonOfInterestID, peopleToFollow, currentPersonOfInterest)
 
}

const compareToFile = (currentTweetID, currentPersonOfInterestID, peopleToFollow, currentPersonOfInterest) => {
    ...
    if (currentTweetID == currentPersonOfInterestID) {
        ...
        console.log("Tweet is new and not on record. Responding to tweet.")
        replyToTweet(currentTweetID, currentPersonOfInterest, counter)
    }
    else {
...
const restartLoop = (peopleToFollow) => {
    ...
    setTimeout(function() {
       ...
       if (counter < Object.keys(peopleToFollow).length-1) {
        console.log(Object.keys(peopleToFollow).length-1)
